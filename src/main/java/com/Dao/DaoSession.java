...
public class DaoSession {
	...
	public static boolean updateSession(TreeSet<BeanSession> list) throws SQLException {
		...
		try (Connection con = HikariCPDataSource.getConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
	...
	public int validateSession(String sessionId, Cookie[] cookies) throws Exception {
		BeanSession obj = new BeanSession();
		obj.setSessionid(sessionId);
		TreeSet<BeanSession> sessionSet=SessionData.getSessionSet();
		if(sessionSet!=null) {
		for (BeanSession session : sessionSet) {
			if (session.getSessionid().equals(sessionId)&& session.getUser_id()!=0) {
	...
	public boolean invalidateSession(String sessionId) throws SQLException, IOException, NoSuchFieldException, IllegalAccessException {
		...
		int k = QueryLayer.buildDeleteQuery(Tables.SESSION, new Condition(Session.sessionid, "=", false), s,new Column[] {Session.sessionid});
